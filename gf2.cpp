...
struct Array {
    ...
    {
	InsertMany(&item, index, 1);
    }
    void Add(T item)
    {
	Insert(item, length);
    }
    void AddMany(const T *newItems, size_t newCount)
    {
	InsertMany(newItems, length, newCount);
    }
    void Free()
    {
	free(array);
    ...
    {
	if (index != length - 1) array[index] = Last();
	Pop();
    }
};
...
void InterfaceLayoutCreate(UIElement *parent) {
	...
	for (uintptr_t i = 0; copy[i]; i++) if (copy[i] == ',') copy[i] = '\t';
	    else if (copy[i] == ')') copy[i] = 0;
